...
class ChernogramProfile {
  ...
  factory ChernogramProfile.fromJson(Map<String, dynamic> json) =>
      ChernogramProfile(
...
class TunnelAttachment {
  ...
  factory TunnelAttachment.fromJson(Map<String, dynamic> json) =>
      TunnelAttachment(
...
class ChernogramMessage {
  ...
  factory ChernogramMessage.fromJson(Map<String, dynamic> json) =>
      ChernogramMessage(
        id: json['id']?.toString() ?? _randomId(),
...
class ChernogramTunnel {
  ...
  String get landingUrl =>
      '$temporaryLandingBase?invite=${Uri.encodeQueryComponent(inviteToken)}';
  ...
  factory ChernogramTunnel.fromJson(Map<String, dynamic> json) =>
      ChernogramTunnel(
        id: json['id']?.toString() ?? _randomId(),
        ...
        messages: ((json['messages'] as List?) ?? const [])
            ...
            .map(
              (item) =>
                  ChernogramMessage.fromJson(Map<String, dynamic>.from(item)),
            )
            .toList(),
      );
...
class ChernogramStore {
  ...
  static Future<ChernogramProfile?> loadProfile() async {
    ...
    return decoded is Map
        ? ChernogramProfile.fromJson(Map<String, dynamic>.from(decoded))
        : null;
  }
  ...
  static Future<List<ChernogramTunnel>> loadTunnels() async {
    ...
    return decoded
        .whereType<Map>()
        .map(
          (item) => ChernogramTunnel.fromJson(Map<String, dynamic>.from(item)),
        )
        .toList();
  }
...
class _ChernogramV06State extends State<ChernogramV06> {
  ...
  String? _tokenFromUri(Uri uri) {
    ...
    if (uri.queryParameters['invite']?.isNotEmpty == true)
  ...
  Future<void> _joinToken(String token) async {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              widget.ru
                  ? 'QR не является приглашением Чернограма.'
                  : 'This QR is not a Chernogram invite.',
            ),
          ),
        );
      }
    ...
    final accepted = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text(
          widget.ru ? 'Подключиться к туннелю?' : 'Join this tunnel?',
        ),
        content: Text(
          widget.ru
  ...
  Widget build(BuildContext context) {
    if (_loading) {
      return const Scaffold(
        body: Center(child: ChernogramLogo(size: 118, withPlate: true)),
      );
    }
    final pages = [
      TunnelsV06Screen(
    ...
    return Scaffold(
      appBar: AppBar(
        title: BrandHeader(
          subtitle: widget.ru
              ? 'ПРИВАТНАЯ СЕТЬ ВАШИХ УСТРОЙСТВ'
              : 'YOUR PRIVATE DEVICE NETWORK',
        ),
        actions: [
          IconButton(
...
class TunnelsV06Screen extends StatelessWidget {
  ...
  Future<void> _create(BuildContext context) async {
    if (profile == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            ru ? 'Сначала создайте профиль.' : 'Create your profile first.',
          ),
        ),
      );
      return;
            ...
            children: [
              ...
              SwitchListTile(
                ...
                subtitle: Text(
                  isPublic
                      ? (ru
                            ? 'Любой с действующим приглашением войдёт сразу.'
                            : 'Anyone with a valid invite can join immediately.')
                      : (ru
                            ? 'Каждый вход подтверждает владелец.'
                            : 'The owner approves every join request.'),
                ),
              ),
              SizedBox(
                ...
                child: FilledButton(
                  onPressed: () {
                    ...
                    if (name.length >= 3)
                      Navigator.pop(context, (name, isPublic));
                  },
  ...
  Future<void> _open(BuildContext context, ChernogramTunnel tunnel) async {
    ...
    final updated = await Navigator.push<ChernogramTunnel>(
      ...
      MaterialPageRoute(
        builder: (_) =>
            TunnelChatV06(ru: ru, profile: profile!, tunnel: tunnel),
      ),
    );
        ...
        Container(
          ...
          decoration: BoxDecoration(
            gradient: const LinearGradient(
              colors: [Color(0xFF351705), Color(0xFF17110D)],
            ),
          ...
          child: Column(
            ...
            children: [
              Text(
                ru ? 'Туннели Чернограма' : 'Chernogram tunnels',
              ...
              Text(
                ru
                    ? 'Чат, любые файлы, звонки и доступ к устройствам без центрального хранилища контента.'
                    : 'Chat, any files, calls and device access without central content storage.',
          ...
          Padding(
            ...
            child: Column(
              children: [
                ...
                const SizedBox(height: 12),
                Text(
                  ru ? 'Создайте первый туннель' : 'Create your first tunnel',
            ...
            Card(
              child: ListTile(
                leading: CircleAvatar(
                  ...
                  child: Icon(
                    tunnel.isPublic ? Icons.public : Icons.lock_outline,
                  ),
                ),
                title: Text(
                  tunnel.name,
                ...
                subtitle: Text(
                  '${tunnel.isPublic ? (ru ? 'Публичный' : 'Public') : (ru ? 'Приватный' : 'Private')} • ${tunnel.messages.length}',
                ),
...
class _TunnelChatV06State extends State<TunnelChatV06> {
    ...
    if (mounted) {
      setState(() {
        ...
        _networkError = session.connected
            ? null
            : (widget.ru
                  ? 'Ожидание телефона владельца…'
                  : 'Waiting for the owner phone…');
      });
    }
      ...
      setState(() {
        ...
        if (active) {
          ...
          _networkError =
              ...
              (widget.ru
                  ? 'Соединение временно потеряно. Переподключаемся…'
          ...
          children: [
            _AttachAction(
              ...
              onTap: () {
                ...
                _pickFiles(extensions: ['jpg', 'jpeg', 'png', 'webp', 'heic']);
              },
            ),
            _AttachAction(
              ...
              onTap: () {
                Navigator.pop(context);
                _pickFiles(extensions: ['mp4', 'mov', 'mkv', 'webm']);
              },
            ),
            _AttachAction(
            ...
            _AttachAction(
              ...
              onTap: () {
                Navigator.pop(context);
                _pickFiles(extensions: ['zip', 'rar', '7z', 'tar', 'gz']);
              },
            ),
            _AttachAction(
  ...
  Future<String> _networkInviteUrl() async {
    ...
    if (widget.profile.id == _tunnel.ownerId) {
      ...
      endpoint =
          _network?.endpoint ?? await LanTunnelDirectory.load(_tunnel.id);
    }
  ...
  Future<void> _copyInvite() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          widget.ru
              ? 'Сетевая ссылка приглашения скопирована'
              : 'Network invite link copied',
        ),
      ),
    );
  }
  ...
  Future<void> _settings() async {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      permissionTile(
                        ...
                        onChanged: (next) =>
                            permissions = permissions.copyWith(canInvite: next),
                      ),
                      permissionTile(
                        icon: Icons.history,
    ...
    setState(() {
      _tunnel = _tunnel.copyWith(
        ...
        inviteSecret: updated.revoke ? _randomId(32) : _tunnel.inviteSecret,
      );
  ...
  Future<void> _showQr() async {
    ...
    await showDialog<void>(
      ...
      builder: (context) => AlertDialog(
        title: Text(
          widget.ru ? 'QR прямого подключения' : 'Direct connection QR',
        ),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
        ...
        actions: [
          ...
          FilledButton(
            ...
            child: Text(widget.ru ? 'Поделиться' : 'Share'),
          ),
        ],
      ),
    );
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            '${message.author} • ${TimeOfDay.fromDateTime(message.sentAt).format(context)}',
                            style: const TextStyle(
                              fontSize: 9,
  ...
  Widget build(BuildContext context) {
    ...
    if (!file.existsSync()) {
      return Row(
        children: [
          ...
          Expanded(
            child: Text(
              ru ? 'Файл удалён с устройства' : 'File was removed from device',
            ),
          ),
        ],
      );
    ...
    if (attachment.kind == AttachmentKind.image) {
      return ClipRRect(
        ...
        child: Image.file(file, width: 250, height: 190, fit: BoxFit.cover),
      );
    }
  ...
  Future<void> _toggle() async {
    if (_playing) {
      ...
      if (_player.audioSource == null)
        await _player.setFilePath(widget.attachment.path);
      await _player.play();
    }
      ...
      Expanded(
        child: Column(
          ...
          children: [
            ...
            Text(
              _fileSize(widget.attachment.size),
              style: const TextStyle(fontSize: 10, color: Colors.white54),
            ),
          ],
        ),
...
class _ChernogramScannerState extends State<ChernogramScanner> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      title: Text(widget.ru ? 'Сканировать туннель' : 'Scan a tunnel'),
    ),
    body: Stack(
      fit: StackFit.expand,
      children: [
...
class FilesV06Screen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final attachments = tunnels
        .expand((tunnel) => tunnel.messages)
        .map((message) => message.attachment)
        .whereType<TunnelAttachment>()
        .toList()
        .reversed
        .toList();
    return ListView(
      ...
      children: [
        Text(
          ru ? 'Файлы ваших туннелей' : 'Files in your tunnels',
          style: const TextStyle(fontSize: 23, fontWeight: FontWeight.w900),
        ),
        const SizedBox(height: 6),
        Text(
          ru
              ? 'Чернограм хранит ссылки на локальные оригиналы. Если оригинал удалён — предпросмотр исчезает.'
              : 'Chernogram keeps local references. If the original is removed, the preview disappears.',
          style: const TextStyle(color: ChernogramColors.textSoft),
        ),
        ...
        Wrap(
          ...
          children: [
            ...
            _FileChip(
              icon: Icons.movie_outlined,
              text: ru ? 'Видео' : 'Videos',
            ),
            _FileChip(icon: Icons.music_note, text: ru ? 'Музыка' : 'Music'),
            _FileChip(
              icon: Icons.description_outlined,
              text: ru ? 'Документы' : 'Documents',
            ),
            _FileChip(
              icon: Icons.folder_zip_outlined,
              text: ru ? 'Архивы' : 'Archives',
            ),
          ],
        ),
        ...
        if (attachments.isEmpty)
          Center(
            child: Padding(
              ...
              child: Text(
                ru
                    ? 'В туннелях пока нет файлов.'
                    : 'There are no tunnel files yet.',
              ),
            ),
          )
        else
              ...
              child: ListTile(
                ...
                subtitle: Text(
                  File(attachment.path).existsSync()
                      ? _fileSize(attachment.size)
                      : (ru ? 'Удалён с устройства' : 'Removed from device'),
                ),
              ),
            ),
      ],
...
class DevicesV06Screen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final routes = [
      (
        Icons.wifi,
        ru ? 'Локальная сеть' : 'Local network',
        ru
            ? 'Прямой канал без интернета'
            : 'Direct connection without internet',
      ),
      (
        Icons.wifi_tethering,
        'Wi‑Fi Direct',
        ru ? 'Для быстрой передачи рядом' : 'Fast nearby transfers',
      ),
      (
        Icons.bluetooth,
        'Bluetooth',
        ru
            ? 'Обнаружение и резерв для малых файлов'
            : 'Discovery and small-file fallback',
      ),
      (
        Icons.public,
        'WebRTC P2P',
        ru ? 'Интернет, чат и звонки' : 'Internet, chat and calls',
      ),
      (
        Icons.swap_horiz,
        'Relay',
        ru
            ? 'Зашифрованный резерв при сложном NAT'
            : 'Encrypted fallback behind difficult NAT',
      ),
    ];
    return ListView(
      padding: const EdgeInsets.fromLTRB(14, 10, 14, 30),
      children: [
        ...
        Card(
          child: ListTile(
            ...
            subtitle: Text(
              profile == null
                  ? (ru ? 'Создайте профиль' : 'Create a profile')
                  : 'ID ${profile!.id}',
            ),
        ...
        const SizedBox(height: 14),
        Text(
          ru ? 'Маршруты соединения' : 'Connection routes',
          style: const TextStyle(fontWeight: FontWeight.w900),
        ),
        const SizedBox(height: 8),
        ...
        Text(
          ru
              ? 'Чернограм будет автоматически выбирать лучший доступный маршрут и переключаться между ними без участия пользователя.'
              : 'Chernogram will automatically choose the best available route and switch between them.',
          style: const TextStyle(color: ChernogramColors.textSoft),
        ),
      ],
    );
...
class _ProfileV06ScreenState extends State<ProfileV06Screen> {
  ...
  Future<void> _save() async {
    ...
    final error = ChernogramNicknameRules.validate(nickname, ru: widget.ru);
    if (error != null) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          widget.ru ? 'Профиль сохранён локально.' : 'Profile saved locally.',
        ),
      ),
    );
  }
    ...
    children: [
      Center(
        child: Stack(
          ...
          children: [
            LocalProfileAvatar(
              key: ValueKey('${widget.profile?.id}:$_avatarRevision'),
              profileId: widget.profile?.id,
              nickname: widget.profile?.nickname ?? '',
              size: 104,
            ...
            Container(
              ...
              child: IconButton(
                tooltip: widget.ru ? 'Выбрать аватарку' : 'Choose avatar',
      ...
      Center(
        child: Text(
          widget.profile == null
              ? (widget.ru ? 'Локальный профиль' : 'Local profile')
...
class _InviteContactsSheetV061State extends State<InviteContactsSheetV061> {
    ...
    return SizedBox(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Text(
              widget.ru ? 'Пригласить из контактов' : 'Invite from contacts',
              style: Theme.of(
                context,
            ...
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintText: widget.ru ? 'Найти человека' : 'Find a person',
              ),
            ),
            const SizedBox(height: 8),
            Expanded(
