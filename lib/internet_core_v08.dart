...
class InternetEventV08 {
  final String type;
  final Map<String, dynamic> data;

  const InternetEventV08(
    this.type, [
    this.data = const <String, dynamic>{},
  ]);
}

class _PendingEnvelopeV08 {
  final String kind;
  final Map<String, dynamic> data;

  const _PendingEnvelopeV08(this.kind, this.data);
}

class InternetTunnelSessionV08 {
  ...
  final StreamController<InternetEventV08> _events =
      StreamController<InternetEventV08>.broadcast(sync: true);
  ...
  final List<_PendingEnvelopeV08> _outbox = <_PendingEnvelopeV08>[];
  ...
  Stream<InternetEventV08> get events => _events.stream;
  ...
  Future<void> _handleSocketMessage(dynamic raw) async {
    try {
      ...
      // A damaged relay packet must not stop the live stream.
    }
  }
  ...
  Future<void> _handleEncryptedPacket(String encrypted) async {
    ...
    _emitPresence(peerId: sender, peerName: senderName);
  ...
  Future<void> _sendEnvelope(
    ...
    try {
      ...
      if (queueOnFailure) {
        ...
        final messageId = (data['message'] as Map?)?['id']?.toString();
        final duplicate = _outbox.any((item) {
          if (actionId != null && actionId.isNotEmpty) {
            return item.data['actionId']?.toString() == actionId;
          }
        ...
        if (!duplicate) {
          _outbox.add(_PendingEnvelopeV08(kind, Map<String, dynamic>.from(data)));
          if (_outbox.length > 160) _outbox.removeAt(0);
        }
  ...
  Future<void> _flushOutbox() async {
    ...
    final pending = List<_PendingEnvelopeV08>.from(_outbox);
    _outbox.clear();
  ...
  void _emitPresence({String? peerId, String? peerName}) {
  ...
  void _emit(String type, [Map<String, dynamic> data = const {}]) {
    if (!_events.isClosed) _events.add(InternetEventV08(type, data));
  }
...
class InternetRelayV08 {
  static final Map<String, InternetTunnelSessionV08> _sessions =
      <String, InternetTunnelSessionV08>{};

  static InternetTunnelSessionV08? session(String tunnelId) =>
      _sessions[tunnelId];

  static Future<InternetTunnelSessionV08> open({
    ...
    final session = InternetTunnelSessionV08(
      tunnelId: tunnelId,
      secret: secret,
      profileId: profileId,
      nickname: nickname,
